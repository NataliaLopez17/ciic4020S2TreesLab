...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> implements Cloneable {
	...
	/**
	 * Creates a root for an empty tree, storing the specified element. The tree
	 * must be empty for this operation to be valid.
	 * 
	 * @param e reference to the element to be added
	 * @return position where the element is placed, which also becomes the root of
	 *         the tree.
	 * @throws IllegalStateException throws this exception if the tree is not empty.
	 *                               Such operation is not valid on a non empty
	 *                               tree.
	 */
	 ...
	 * 
	 * @param p The position whose left child will be the new position containing
	 *          the specified element.
	 * @param e The new element to be placed in that new position.
	 * @return The new position that is created.
	 * @throws IllegalArgumentException If p is not valid pas per the validate
	 *                                  method, of if p already has a left child in
	 *                                  the tree.
	 */
	...
	/**
	 * Same as the above, but to add a right child of p, which shall containg
	 * element e.
	 * 
	 * @param p
	 * @param e
	 ...
	 * added are supposed to be independent from the tree where they are to be
	 * inserted. At the end, both trees to be added will be altered so as to become
	 * empty trees. Note that, if successful, the size of this tree should be
	 * incremented by adding the sizes of the original trees that are attached.
	 * 
	...
	/**
	 * Removes the node at a given position. Replaces it with its current child (if
	 * any).
	 * 
	 * @param p The position to be removed (or corresponding to the node to be
	 *          removed.
	 * @return The element at the removed position.
	 * @throws IllegalArgumentException if p is not a valid position or if p has two
	 *                                  children. This remove is not valid for nodes
	 *                                  having two children.
	 */
	...
	/**
	 * SEE ALSO METHOD set in textbook. Work an implementation on your own, but not
